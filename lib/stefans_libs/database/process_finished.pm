package stefans_libs::database::process_finished;
...
sub new {
    ...
    bless $self, $class if ( $class eq "stefans_libs::database::process_finished" );
...
sub  init_tableStructure {
     ...
     $hash->{'UNIQUES'}   = [];
     $hash->{'variables'} = [];
     $hash->{'table_name'} = "procTOfinished";
     ...
     push ( @{$hash->{'UNIQUES'}}, [ 'module', 'program' ]);

     $self->{'table_definition'} = $hash;
     $self->{'UNIQUE_KEY'} = [ 'program' ];
...
sub for_module{
	my ( $self, $module ) = @_;
	$self->{'use_this_sql'} = "select id, module, program from ".$self->TableName(). " where module = '$module'";
	return $self-> get_data_table_4_search( {
	  	'search_columns' => [ 'id', 'module', 'program' ],
	  	'where' => [  ],
	  }, );
}
