...
use stefans_libs::database::scientistTable::CatalystUser;
use stefans_libs::WEB_Objects::scientistTable;
use stefans_libs::database::scientistTable::temporary_banned;
...
sub get_as_object {
	my ( $self, $id ) = @_;
	my $obj = stefans_libs_WEB_Objects_scientistTable->new($self);
...
sub Get_id_for_name {
	...
	my $sth =
	  $self->dbh()
	  ->prepare(
		'select id from ' . $self->TableName() . " where username = '$name'" );
	$sth->execute();
	my $id;
	$sth->bind_columns( \$id );
	$sth->fetch();
...
sub check_pw {
	...
	if ( defined $hash ) { ##re-check!
		...
		if ( defined $c ){
		$c->user(
			stefans_libs_database_scientistTable_CatalystUser->new($user, $self) );
		$c->session->{'user'} = $user;
		}
		return 1;
	}
